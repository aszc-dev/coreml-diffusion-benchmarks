...
- **R6.4** The harness MUST report, per cell: the **median** GPU power and **median** ANE power over the active window (each baseline-subtracted per R6.2), and an **energy-per-image** figure (power × wall time across the timed iterations). The median (not the mean) is required because the active/baseline window bounds are wall-clock-aligned across two processes (R6.3) and so drift by up to one sample interval relative to the engine's power ramp; a few leaked full-power samples in the baseline window or near-zero samples in the active window bias a mean by tens of percent, whereas the median is dominated by the steady state on both sides. This is the same rationale as R5.4's median-not-mean latency, and yields the settled per-step cost rather than one amortised over engine spin-up/down.
...
- **R11.5** The per-run manifest MUST include a top-level **`schema_version`** integer. Additive field changes do not bump it; removing or renaming a captured field MUST bump it. A maintainer-side validator MUST refuse a bundle whose `schema_version` exceeds the supported value.
- **R11.6** Host hardware identity MUST be captured at the level of: chip brand + family + variant (e.g. `Apple M2 Pro` → family `M2`, variant `Pro`), performance / efficiency / logical CPU core counts, GPU core count, ANE presence, RAM bytes, and the `hw.model` identifier. A non-PII `host_id_hash` (salted SHA-256 of `kern.uuid`) MUST be recorded so distinct hosts can be deduped without leaking the underlying UUID.
- **R11.7** OS identity MUST include macOS product name, product version, **build version** (`sw_vers -buildVersion`), kernel string, and `nvram boot-args`. The Rosetta translation flag MUST be recorded; the harness MUST refuse to run when `sysctl.proc_translated == 1`.
- **R11.8** Repository state of the harness checkout MUST be captured: current git commit SHA, branch, dirty flag, the list of dirty paths (capped at 200), upstream URL, and `git describe`. A `dirty == true` run MUST be flagged in the manifest and surfaced in every generated table caption; it MUST NOT be aborted.
- **R11.9** All three `uv.lock` files (harness, `envs/apple-ct8`, `envs/team-ct9`) MUST be content-hashed (SHA-256) into the manifest, and the **full resolved package set** of the harness environment MUST be recorded, not a hand-picked subset. The conversion environments MUST also stamp the SHA-256 of the conversion driver script they consumed.
- **R11.10** Runtime conditions MUST be snapshotted at run start AND run end: AC vs battery state, battery percent, low-power mode, sleep-disabled, displaysleep setting, caffeinate PIDs, loadavg (1/5/15 min), uptime, process count, and the thermal pressure snapshot. Per-cell thermal at cell start and cell end MUST be recorded so drift across the matrix is auditable.
- **R11.11** Determinism inputs MUST be hashed and stamped per record: the seed, the RNG kind (`numpy.default_rng(PCG64)`), batch size, the materialized latent SHA-256, the materialized text-embedding SHA-256, the timestep, the shared-input `.npz` file SHA-256, the text-embedding source label, and the checkpoint SHA-256.
- **R11.12** The power sampler block MUST record `powermetrics` version banner, sample interval, sampler list, baseline-window seconds, the sudo-cache state at sampler start, the total sample count, the count of samples that fell into a baseline window, and the SHA-256 of the raw plist.
- **R11.13** Behavior-affecting environment variables MUST be captured into the manifest **after** the harness's own module-level overrides apply (`PYTORCH_*`, `TORCH_*`, `COREML_*`, `MLX_*`, `MPS_*`, `OMP_*`, `MKL_*`, `VECLIB_*`, `ACCELERATE_*`, `HF_*`, `HUGGINGFACE_*`, `TRANSFORMERS_*`, `DIFFUSERS_*`, `TOKENIZERS_*`, `TQDM_*`, `PYTHONHASHSEED`, `PYTHONWARNINGS`, `NUMEXPR_*`, `BLIS_*`). A digest of the captured map MUST appear on every record.
- **R11.14** A contributor submission bundle (produced by `sdbench report`) MUST contain: the manifest JSON, the results JSONL, the raw `*-powermetrics.plist`, the generated tables, the `matrix.yaml` consumed, a bundle README explaining provenance digest verification, and a `SCHEMA.md` documenting the bundle's schema version. Anonymization MUST be opt-in, MUST strip filesystem-local PII (`checkpoint_path`, `repo.upstream_url`, `repo.dirty_files`) and re-hash `host_id_hash` with a contributor-supplied salt, and MUST preserve every field needed for reproducibility (checkpoint SHA, seed, latent/text-embedding hashes, toolchain pins).
...
- **A11** Every record carries a `schema_version`; `sdbench validate-report` accepts a bundle produced from a clean run on the current harness and refuses one whose `schema_version` exceeds the supported value (R11.5).
- **A12** `sdbench report` produces a self-contained bundle (and a `.zip`) which, when handed to the maintainer, re-derives the manifest's `provenance_digest` and verifies cleanly under `validate-report` — including the latent / text-embedding SHA-256 consistency check (R11.11, R11.14).
- **A13** A run started under Rosetta is refused; a run started with `repo.dirty == true` is recorded with that flag visible in every table caption and in the bundle README (R11.7, R11.8).
